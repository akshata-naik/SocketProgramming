...
*       FUNCTION NAME: main
*
*       DESCRIPTION:The patient-1 sends the authentication 
...
int main()
 {	
    ...
    printf("\n Phase 1: Patient 1 has port number %d and IP address %s. \n",ntohs(myaddrA.sin_port),inet_ntoa(myaddrA.sin_addr));
	freeaddrinfo(servinfo);
	if (send(sockfd, msg, strlen(msg), 0) == -1)
		perror("send");
	
	printf("\n Phase 1: Authentication request from Patient1 with username %s and password %s has been sent to the Health Center Server.\n ", data1, data2);
	...
	if((strcmp(msg,"Success"))==0)
	{
		...
		if((strcmp(msg,"notavailable"))!=0)
		{
			...
			printf("\n Phase 2: The requested appointment is available and reserved to Patient1. The assigned doctor port number is %s. \n",docport);
		}
		else
		{
	   ...
	   if (bind(fd, (struct sockaddr *)&myaddr, sizeof(myaddr)) < 0) 
       {
        ...
        printf(" \n Phase 3: Patient 1 has dynamic UDP port number %d and IP address ", ntohl(myaddrA.sin_port) );
	...
	msg[numbytes]='\0';
	//printf("%d", numbytes);
        printf("\n Phase 3: Patient 1 receives %s$ estimation cost from doctor with port number %s and IP address %s. \n ", msg, docport, sdd);
